...
parser.add_option('--activation-type', dest='activation_type', 
                   help='Select type of activation function : (<Sigmoid>|<Tanh>|<ReLU>|<SoftHinge>|<Maxout>|<Pnorm>) [default: %default]', 
                   default='<Sigmoid>', type='string');
parser.add_option('--maxout-group', dest='maxout_group', 
                   help='Maxout unit group size (0 = no maxout) [default: %default]',
                   default=0, type='int');
parser.add_option('--pnorm-p', dest='pnorm_p', 
                   help='P norm configuration (Enabled by --activation-type <Pnorm>) [default: %default]',
                   default=2.0, type='float');
parser.add_option('--activation-norm', dest='activation_norm',
                   help='Apply normalization on activations [default: %default]',
...
if o.maxout_group > 0 and o.activation_type == "<Maxout>":
  print "<Maxout> <InputDim> %d <OutputDim> %d" % (num_hid_neurons, (num_hid_neurons / o.maxout_group))
  if o.activation_norm:
    print "<Normalize> <InputDim> %d <OutputDim> %d" % ((num_hid_neurons / o.maxout_group), (num_hid_neurons / o.maxout_group))
elif o.activation_type == "<Pnorm>":
  assert(o.maxout_group > 0)
  print "<Pnorm> <InputDim> %d <OutputDim> %d" % (num_hid_neurons, (num_hid_neurons / o.maxout_group))
  if o.activation_norm:
    print "<Normalize> <InputDim> %d <OutputDim> %d" % ((num_hid_neurons / o.maxout_group), (num_hid_neurons / o.maxout_group))
else:
  print "%s <InputDim> %d <OutputDim> %d" % (o.activation_type, num_hid_neurons, num_hid_neurons)
  if o.activation_norm:
    print "<Normalize> <InputDim> %d <OutputDim> %d" % (num_hid_neurons, num_hid_neurons)

# Internal AffineTransforms,
for i in range(num_hid_layers-1):
  if o.maxout_group > 0 and o.activation_type == "<Maxout>":
    print "<AffineTransform> <InputDim> %d <OutputDim> %d <BiasMean> %f <BiasRange> %f <ParamStddev> %f <MaxNorm> %f" % \
        ((num_hid_neurons / o.maxout_group), num_hid_neurons, o.hid_bias_mean, o.hid_bias_range, \
    ...
    print "<AffineTransform> <InputDim> %d <OutputDim> %d <BiasMean> %f <BiasRange> %f <ParamStddev> %f <MaxNorm> %f" % \
        ((num_hid_neurons / o.maxout_group), num_hid_neurons, o.hid_bias_mean, o.hid_bias_range, \
         (o.param_stddev_factor * Glorot(num_hid_neurons, num_hid_neurons)), o.max_norm)
    assert(o.maxout_group > 0)
    assert(o.activation_type == "<Pnorm>")
    print "<Pnorm> <InputDim> %d <OutputDim> %d" % (num_hid_neurons, (num_hid_neurons / o.maxout_group))
    if o.activation_norm:
      print "<Normalize> <InputDim> %d <OutputDim> %d" % ((num_hid_neurons / o.maxout_group), (num_hid_neurons / o.maxout_group))
  else:
...
# Last AffineTransform (10x smaller learning rate on bias)
if o.maxout_group > 0 and o.activation_type == "<Maxout>":
  num_hid_neurons = num_hid_neurons / o.maxout_group
elif o.maxout_group > 0 and o.activation_type == "<Pnorm>":
  num_hid_neurons = num_hid_neurons / o.maxout_group
