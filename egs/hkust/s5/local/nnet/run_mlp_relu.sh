...
if [ $stage -le 1 ]; then
  ...
  for nn in $(seq -w $nn_depth); do
    ...
    $cuda_cmd $dir/log/mlp.$nn.log \
      ...
      --proto-opts "--activation-type <ReLU>" \
...
if [ $stage -le 2 ]; then
  ...
  pretrain_dir=exp/dnn5a_relu
  dir=exp/dnn5a_relu_pretrain_dnn_exp
  mkdir -p $dir $dir/log 
  feature_transform=exp/dnn5b_pretrain-dbn/tr_splice$splice-${splice_step}_cmvn-g.nnet
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    steps/nnet/train_exp.sh --feature-transform $feature_transform --dbn $dnn --prepend false \
    --hid-layers 0 --initial-learn-rate 0.002 --final-learn-rate 0.0002 --max-iters 15 \
    $data_fmllr/train_tr90 $data_fmllr/train_cv10 data/lang $ali $ali $dir || exit 1;
