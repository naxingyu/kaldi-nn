...
if [ $stage -le 1 ]; then
  ...
  for nn in $(seq -w 4 $nn_depth); do
    ...
    $cuda_cmd $dir/log/mlp.$nn.log \
      steps/nnet/train.sh --hid-layers 1 --hid-dim 2400 --learn-rate 0.001 \
      --feature-transform $feature_transform --train-opts "--max-iters 1" \
      --proto-opts "--activation-type <Maxout> --maxout-group 3 --activation-norm" \
...
if [ $stage -le 2 ]; then
  ...
  pretrain_dir=exp/dnn5a_maxout
  dir=exp/dnn5a_maxout_pretrain_dnn
  mkdir -p $dir $dir/log
  feature_transform=exp/dnn5b_pretrain-dbn/tr_splice$splice-${splice_step}_cmvn-g.nnet
  dnn=$dir/$nn_depth.dnn
  ali=${gmmdir}_ali
  nnet-copy --remove-first-components=3 $pretrain_dir/$nn_depth.mlp $dnn
  (tail --pid=$$ -F $dir/log/train_nnet.log 2>/dev/null)& # forward log
  $cuda_cmd $dir/log/train_nnet.log \
    steps/nnet/train.sh --feature-transform $feature_transform --dbn $dnn \
