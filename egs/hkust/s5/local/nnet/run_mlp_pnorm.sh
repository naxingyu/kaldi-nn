...
if [ $stage -le 1 ]; then
  ...
  for nn in $(seq -w 4 $nn_depth); do
    ...
    $cuda_cmd $dir/log/mlp.$nn.log \
      steps/nnet/train.sh --hid-layers 1 --hid-dim 6400 --learn-rate 0.0005 \
      --feature-transform $feature_transform --train-opts "--max-iters 1" \
      --proto-opts "--activation-type <Pnorm> --pnorm-p 2.0 --maxout-group 8 --activation-norm" \
...
if [ $stage -le 2 ]; then
  ...
  pretrain_dir=exp/dnn5a_pnorm
  dir=exp/dnn5a_pnorm_pretrain_dnn
  mkdir -p $dir $dir/log
  feature_transform=exp/dnn5b_pretrain-dbn/tr_splice$splice-${splice_step}_cmvn-g.nnet
