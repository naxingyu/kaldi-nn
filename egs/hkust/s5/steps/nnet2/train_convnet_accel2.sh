...
pool_step=3           # step of pooling after the first convolutional layer
...
if [ $stage -le -2 ]; then
  ...
  num_pool=$[1+($num_patch1-$pool_size)/$pool_step]
...
MaxpoolingComponent input-dim=$conv_out_dim1 output-dim=$pool_out_dim pool-size=$pool_size pool-step=$pool_step pool-stride=$num_filters1
