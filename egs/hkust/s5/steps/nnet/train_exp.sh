...
# TRAINING SCHEDULER
initial_learn_rate=0.008   # initial learning rate
final_learn_rate=0.0008
max_iters=15
...
if [ ! -z "$feature_transform" ]; then
  ...
  case $feat_type in
    ...
    transf)
      ...
      feat_dim=$(feat-to-dim "$feats_tr nnet-forward --use-gpu=yes 'nnet-concat $feature_transform_old \"transf-to-nnet $transf - |\" - |' ark:- ark:- |" -)
...
if [[ -z "$nnet_init" && -z "$nnet_proto" ]]; then
  ...
  #input-dim
  num_fea=$(feat-to-dim "$feats_tr nnet-forward --use-gpu=yes $feature_transform ark:- ark:- |" - )
  { #optioanlly take output dim of DBN
    [ ! -z $dbn ] && num_fea=$(nnet-forward --use-gpu=yes "nnet-concat $feature_transform $dbn -|" "$feats_tr" ark:- | feat-to-dim ark:- -)
...
steps/nnet/train_scheduler_exp.sh \
  --feature-transform $feature_transform \
  --initial-learn-rate $initial_learn_rate --final-learn-rate $final_learn_rate \
  --max-iters $max_iters --randomizer-seed $seed \
  ${train_opts} \
...
if $prepend; then
  echo "Preparing feature transform with non-dnn layers for RBM pre-training."
  nnet-concat $dir/final.feature_transform "nnet-copy --remove-last-layers=$(((hid_layers+1)*2)) $dir/final.nnet - |" \
    $dir/final.feature_transform_$network_type 2>$dir/log/concat_transf_$network_type.log || exit 1
fi
