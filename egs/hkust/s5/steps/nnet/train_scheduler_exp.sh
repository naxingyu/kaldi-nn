...
# training
for iter in $(seq -w 0 $max_iters); do
  ...
  ilr=$initial_learn_rate;
  flr=$final_learn_rate;
  learn_rate=$(perl -e  "print (($iter >= $max_iters ? $flr : $ilr*exp($iter*log($flr/$ilr)/$max_iters)));");
