...
struct NnetStreamsTrainerConfig {
  int32 target_delay;
  int32 batch_size;
  int32 num_stream;
  int32 batches_per_phase;
  
  NnetStreamsTrainerConfig(): target_delay(5), batch_size(20), num_stream(4),
                              batches_per_phase(50) { }
  
  void Register (OptionsItf *po) {
    po->Register("target-delay", &target_delay, "Feature-target delay");
    po->Register("batch-size", &batch_size, "Stream batch size");
    po->Register("num-stream", &num_stream, "Number of streams for multi-stream training");
    po->Register("batches-per-phase", &batches_per_phase,
                 "Number of utterances to wait before printing training-set "
                 "objective.");
  }  
};
...
/// weight of the examples (typically equal to the number of examples) and the
/// total logprob objective function.
int64 TrainNnetStreams(const NnetStreamsTrainerConfig &config,
                       Nnet *nnet,
                       SequentialNnetExampleReader *reader,
                       double *tot_weight = NULL,
                       double *tot_logprob = NULL);

struct NnetPerUttTrainerConfig {
  int32 utts_per_phase;
  
  NnetPerUttTrainerConfig(): utts_per_phase(50) { }
  
  void Register (OptionsItf *po) {
    po->Register("utts-per-phase", &utts_per_phase,
                 "Number of utterances to wait before printing training-set "
                 "objective.");
  }  
};
