...
void Nnet::ResetStreamsStates(const std::vector<int32> &stream_reset_flag) {
  for (int32 c = 0; c < NumComponents(); c++) {
    if (GetComponent(c).Type() == "LstmProjectedComponent") {
      LstmProjectedComponent& comp = dynamic_cast<LstmProjectedComponent&>(GetComponent(c));
