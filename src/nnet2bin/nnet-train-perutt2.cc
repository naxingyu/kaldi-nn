...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Train neural network parameters with backprop and stochastic\n"
        "gradient descent using per-utterance mode. Training examples would be\n"
        "produced by nnet-get-egs.\n"
        "\n"
        "Usage:  nnet-train-perutt [options] <model-in> <training-examples-in> <model-out>\n"
        "\n"
        "e.g.:\n"
        "nnet-train-perutt 1.nnet ark:1.egs 2.nnet\n";

    bool binary_write = true;
    bool zero_stats = true;
    int32 srand_seed = 0;
    std::string use_gpu = "yes";
    NnetPerUttTrainerConfig train_config;
    ...
    
    {
      ...
      num_examples = TrainNnetPerUtt(train_config, &(am_nnet.GetNnet()),
				      &example_reader);
