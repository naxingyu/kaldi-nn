...
int main(int argc, char *argv[]) {
  try {
    ...
    int64 num_read = 0, num_written = 0, utt_written = 0;
    ...
    for (; !example_reader.Done(); example_reader.Next(), num_read++) {
      ...
      // suppose the egs are created by nnet-get-egs
      // key = uttid-xx, now retrieve uttid from key
      utt_id = KeyToUtt(key);
      const NnetExample &eg = example_reader.Value();
      for (int32 c = 0; c < count; c++) {
        if (!per_utt)
          index = (random ? Rand() : num_written) % num_outputs;
	else if (utt_id != prev_utt_id) {
	  utt_written++;
	  index = (random ? Rand() : utt_written) % num_outputs;
	}
        if (!copy_eg) {
